...
CREATE TABLE IF NOT EXISTS `keramba` (
  ...
  CONSTRAINT `FK_Owner` FOREIGN KEY (`user_id`)
...
DROP TABLE IF EXISTS `detail_panen`;
CREATE TABLE IF NOT EXISTS `detail_panen` (
  ...
  `biota_id` INT(11) NOT NULL,
  PRIMARY KEY (`activity_id`),
  CONSTRAINT `FK_TabelPanen` FOREIGN KEY (`activity_id`)
  REFERENCES `panen`(`activity_id`) ON DELETE CASCADE ON UPDATE CASCADE,
